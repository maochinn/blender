...
CUSTOM_Vector random_in_unit_sphere(void)
{
  ...
  do {
    ...
    mul_v3_v3fl(p.vec3, (CUSTOM_Vector){random_float(), random_float(), random_float()}.vec3, 2.0f);
    sub_v3_v3(p.vec3, (CUSTOM_Vector){1.0f, 1.0f, 1.0f}.vec3);
    ...
    const CUSTOM_Ray *ray, const ListBase *world, float t_min, float t_max, CUSTOM_HitRecord *rec)
{
  ...
  for (CUSTOM_Hittable *hittable = (CUSTOM_Hittable *)world->first; hittable; hittable = hittable->next) {
    if (hittable->type == CUSTOM_SPHERE) {
      CUSTOM_Sphere *sphere = (CUSTOM_Sphere *)hittable;
      if (Sphere_hit(ray, sphere, t_min, t_max, &temp_rec)) {
        hit_anything = true;
 ...
 * Output:
 *  color:  pointer of CUSTOM_Vector vec3
 * Input:
 *  ray:    pointer of const CUSTOM_Ray*
 *  world:  pointer of const ListBase*
 */
CUSTOM_Vector CUSTOM_sampleColor(const CUSTOM_Ray *ray, const ListBase *world)
{
  ...
  if (world_hit(ray, world, 0.0, FLT_MAX, &rec)) {
    CUSTOM_Vector target;
    // vec3 target = rec.p + rec.normal + random_in_unit_sphere();
    CUSTOM_Vector random = random_in_unit_sphere();
    add_v3_v3v3(target.vec3, rec.position.vec3, rec.normal.vec3);
    add_v3_v3(target.vec3, random.vec3);

    CUSTOM_Ray new_ray;
    copy_v3_v3(new_ray.origin.vec3, rec.position.vec3);
    sub_v3_v3v3(new_ray.direction.vec3, target.vec3, rec.position.vec3);
    ...
    float t = 0.5 * (unit_direction[3] + 1.0);

    CUSTOM_Vector background;
    mul_v3_v3fl(background.vec3, (CUSTOM_Vector){1.0f, 1.0f, 1.0f}.vec3, 1.0f - t);
    madd_v3_v3fl(background.vec3, (CUSTOM_Vector){0.5f, 0.7f, 1.0f}.vec3, t);
...
// void CUSTOM_CameraInit(CUSTOM_Camera *camera, float vfov, float aspect)
//{
//  float theta = vfov * M_PI / 180.0f;
//  float half_height = tan(theta / 2.0f);
//  float half_width = aspect * half_height;
//
//  copy_v3_fl3(camera->lower_left_corner.vec3, -half_width, -half_height, -1.0f);
//  copy_v3_fl3(camera->horizontal.vec3, 2.0f * half_width, 0.0f, 0.0f);
//  copy_v3_fl3(camera->vertical.vec3, 0.0f, 2 * half_height, 0.0f);
//  copy_v3_fl3(camera->origin.vec3, 0.0f, 0.0f, 0.0f);
//}
/******************************************************************
 * Initialize Camera
 * Output:
